...
export const updateWeeklyGoal = async (req, res, next) => {
  try {
    ...
    const stats = await updateWeeklyGoalService(
      req.user.id,
      parseInt(weeklyGoal),
    );
...
export const getWeeklyProgressController = async (req, res, next) => {
  try {
    const stats = await getWeeklyProgressService(req.user.id);
    res.status(200).json({
      status: "success",
      data: stats,
    });
